...
public class ITunesRssRequest extends AbstractRequest {
	...
	public Tracks parse(String jsonString) throws Exception{
        ...
        for (int i = 0; i < array.length(); i++) {          
            ...
            String artworkUrl = imageArray.getJSONObject(imageArray.length()-1).getString("label");
            
            String collectionName  = object.getJSONObject("im:collection").getJSONObject("im:name").getString("label");
            String previewUrl = object.getJSONArray("link").getJSONObject(1).getJSONObject("attributes").getString("href");
